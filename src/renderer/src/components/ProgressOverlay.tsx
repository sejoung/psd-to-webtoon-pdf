  ...
  return (
    <div className="absolute inset-0 z-40 flex items-center justify-center bg-bg/85 backdrop-blur">
      <div className="w-[480px] rounded-card border border-border bg-surface p-6 shadow-2xl">
        <h2 className="text-base font-semibold text-text-primary">병합 중…</h2>
        <p className="mt-1 text-xs text-text-secondary">{phaseLabel(phase)}</p>
        ...
        {progress?.currentFileName && (
          <p className="mt-4 truncate text-sm text-text-primary">
            <span className="text-text-secondary">현재 파일: </span>
            {progress.currentFileName}
          </p>
        )}
        ...
        <div className="mt-4 flex items-baseline gap-4 text-xs text-text-secondary">
          ...
          {remainingMs !== null && (
            <span>예상 남은 시간 {formatDuration(remainingMs)}</span>
          )}
          {skippedCount > 0 && (
            <span className="text-warning">{skippedCount}개 스킵됨</span>
          )}
        </div>
